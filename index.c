    ...
    switch (op)
    {
      ...
      case OP_TOGGLE_READ:
      {
        ...
        if (op == OP_TOGGLE_READ)
        {
          ...
          if (!lmt || !mutt_strn_equal(Context->pattern, "!~R!~D~s", 8))
          {
            snprintf(buf2, sizeof(buf2), "!~R!~D~s%s", lmt ? Context->pattern : ".*");
          }
          else
          {
      ...
      case OP_MAIN_PARENT_MESSAGE:
      {
        ...
        menu->current = mutt_parent_message(ctx_has_limit(Context), cur.e, op == OP_MAIN_ROOT_MESSAGE);
        if (menu->current < 0)
        {
          menu->current = menu->oldcurrent;
        }
